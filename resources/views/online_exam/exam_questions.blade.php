    ...
    <div class="content-wrapper">
        ...
        <div class="row grid-margin">
            <div class="col-lg-12">
                <div class="card">
                    <div class="card-body">
                        <form class="pt-3" id="add-new-question-online-exam" method="POST"
                            action="{{ route('online-exam.add-new-question') }}">
                            <div class="row mb-4">
                                <div class="form-group col-md-4">
                                    <label>{{ __('class') }}</label>
                                    <input type="text" id="add-question-class" value="{{ $class_data['class_name'] }}"
                                        placeholder="{{ __('class') }}" class="form-control" readonly />
                                </div>
                                <div class="form-group col-md-4">
                                    <label>{{ __('subject') }}</label>
                                    <input type="text" id="add-question-subject"
                                        value="{{ $online_exam_db->subject->name . ' - ' . $online_exam_db->subject->type }}"
                                ...
                                <div class="form-group col-md-4">
                                    ...
                                    <input type="text" value="{{ $online_exam_db->title }}"
                            ...
                            <div class="add-new-question-container" style="display:none">
                                <div class="bg-light p-4 rounded">
                                    <div class="form-group mb-4">
                                        <label class="mb-3">{{ __('question_type') }} <span
                                                class="text-danger">*</span></label>
                                        <button type="button"
                                            class="btn btn-danger btn-sm float-right remove-add-new-question">
                                    ...
                                    <div id="simple-question">
                                        ...
                                        <div class="row option-container mb-4">
                                        ...
                                        <div class="add_button mb-4">
                                            <button class="btn btn-dark btn-sm" type="button" id="add-new-option">
                                                <i class="fa fa-plus-circle mr-2"
                                                    aria-hidden="true"></i>{{ __('add_option') }}
                                            </button>
                                        </div>
                                    </div>
                                    <div id="equation-question" style="display: none">
                                        ...
                                        <div class="row equation-option-container mb-4">
                                            <div class="form-group col-md-6">
                                                <label>{{ __('option') }} <span class="option-number">1</span> <span
                                                        class="text-danger">*</span></label>
                                        ...
                                        <div class="add_button_equations mb-4">
                                            <button class="btn btn-dark btn-sm" type="button"
                                                id="add-new-eqation-option">
                                                <i class="fa fa-plus-circle mr-2"
                                                    aria-hidden="true"></i>{{ __('add_option') }}
                                            </button>
                                        </div>
                                ...
                                <div class="mt-4">
                                    <input class="btn btn-theme" id="new-question-add" type="submit"
                                        value={{ __('add') }}>
                                </div>
                            </div>
                        </form>
            ...
            <div class="col-md-6 grid-margin stretch-card">
                <div class="card">
                    <div class="card-body">
                        ...
                        <table aria-describedby="mydesc" data-escape="false" class='table table-striped'
                            id='table_list_exam_questions' data-toggle="table"
                            data-url="{{ route('online-exam-question.get-class-subject-questions', ['online_exam_id' => $online_exam_db->id]) }}"
                            ...
                            data-maintain-selected="true" data-query-params="onlineExamQuestionsQueryParams"
                            ...
                            <thead>
                                <tr>
                                    ...
                                    <th scope="col" data-field="question" data-sortable="false">{{ __('question') }}
                                    </th>
                                    <th scope="col" data-field="image" data-escape="false"
                                        data-formatter="imageFormatter" data-sortable="false" data-align="center">
                                        {{ __('image') }}</th>
                                </tr>
                            </thead>
                        </table>
                                        ...
                                        @foreach ($exam_questions as $questions)
                                            @if ($questions->questions->question_type)
                                                <div class="list-group mb-2">
                                                    <input type="hidden"
                                                        name="assign_questions[{{ $questions->question_id }}][edit_id]"
                                                        value="{{ $questions->id }}">
                                                    <input type="hidden"
                                                        name="assign_questions[{{ $questions->question_id }}][question_id]"
                                                        value="{{ $questions->question_id }}">
                                                    <li id="q{{ $questions->question_id }}"
                                                        class="list-group-item d-flex justify-content-between align-items-center ui-state-default list-group-item-secondary">
                                                        <div class="d-flex align-items-center">
                                                            <span class="mr-3">{{ $questions->question_id }}.</span>
                                                            <div class="question-text">
                                                                <textarea class="equation-editor-inline" name="qc{{ $questions->question_id }}">{{ safe_htmlspecialchars_decode($questions->questions->question) }}</textarea>
                                                            </div>
                                                        </div>
                                                        <div class="d-flex align-items-center">
                                                            <div class="form-group mb-0 mr-2">
                                                                <input type="number" class="form-control"
                                                                    name="assign_questions[{{ $questions->question_id }}][marks]"
                                                                    style="width: 100px" placeholder="Marks"
                                                                    value="{{ $questions->marks }}" min="0">
                                                            </div>
                                                            <a class="btn btn-danger btn-sm remove-row"
                                                                data-edit_id="{{ $questions->id }}"
                                                                data-id="{{ $questions->question_id }}"
                                                                data-related='[
                                                ...
                                                <div class="list-group mb-2">
                                                    ...
                                                    <li id="q{{ $questions->question_id }}"
                                                        ...
                                                        <div class="d-flex align-items-center">
                                                            <span class="mr-3">{{ $questions->question_id }}.</span>
                                                            <span
                                                                class="question-text">{{ safe_htmlspecialchars_decode($questions->questions->question) }}</span>
                                                        </div>
                                                        <div class="d-flex align-items-center">
                                                            <div class="form-group mb-0 mr-2">
                                                                <input type="number" class="form-control"
