...
<script
    src="https://cdnjs.cloudflare.com/ajax/libs/jquery-validate/1.21.0/localization/messages_{{ Session::get('language')->code ?? 'en' }}.min.js">
</script>
...
<script src="{{ asset('/assets/js/custom/bootstrap-table-locale.js') }}"></script>
...
<script>
    ...
    $(document).on('click', '.deletedata', function() {
        const relatedEntities = $(this).data('related'); // array OR undefined
        let extraHtml = '';
        const warningText = $(this).data('warningText');

        if (Array.isArray(relatedEntities) && relatedEntities.length > 0) {
            const entityList = relatedEntities.join(', ');

            extraHtml = `
            ${warningText ? `<p class="text-warning mt-2">${warningText}</p>` : ''}
            <p class="text-danger mt-2">
                This category is linked with <strong>${entityList}</strong>.
